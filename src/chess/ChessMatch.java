...
public class ChessMatch {
	...
	public ChessPiece performChessMovie(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentyPlayer)) {
			unMove(source, target, capturePiece);
			throw new ChessException("You can't put yourself in check");
		}

		check = (testCheck(opponet(currentyPlayer))) ? true : false;

		if (testCheckMate(opponet(currentyPlayer))) {
			checkMate = true;
	...
	private void unMove(Position source, Position target, Piece capturedPiece) {
			...
			for (int i = 0; i < board.getRow(); i++) {
				for (int j = 0; j < board.getColumn(); j++) {
					if (mat[i][j]) {
						...
						unMove(source, target, capturedPiece);
